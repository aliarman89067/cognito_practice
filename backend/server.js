...
async function initializeServer() {
  ...
  jwtSigningKey = await getCognitoJWTPublicKey(
    server.href + "/.well-known/jwks.json "
  );
}
...
app.get("/login", async (req, res) => {
  try {
    ...
    const congnitoLoginURL = client.buildAuthorizationUrl(
      config,
      parameters
    ).href;
...
app.get("/todos", (req, res) => {
  ...
  const isAdmin = JSON.parse(
    Buffer.from(
      req?.signedCookies?.ACCESS_TOKEN?.split(".")[1],
      "base64"
    )?.toString("utf8")
  )["cognito:groups"]?.includes("Admin");
